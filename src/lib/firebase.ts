...
async function testConnection() {
  try {
    ...
    if (error && error.message && error.message.includes('the client is offline')) {
      ...
      // It's normal if the document 'test/connection' doesn't exist, as long as it reached the server
      console.log("Firestore connection test: reached server (document may not exist).");
    }
  }
}
testConnection();
